...
class KernighanLin():
    ...
    def partition(self):
        ...
        os.remove('Kernighan-Lin-master/result.txt')
        for i in self.graph.vertices:
            with open(os.path.join('Kernighan-Lin-master',f'result.txt'),'a+') as f:
                f.write(' '.join([str(i.id),str(i.partition_label),'\n']))
        
def main():
    graph = load_data("Kernighan-Lin-master/data.txt")
